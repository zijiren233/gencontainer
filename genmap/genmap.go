...
func (m *GenMap[K, V]) CopyTo(dst GenMap[K, V]) {
	maps.Copy(dst, *m)
}

func (m *GenMap[K, V]) CopyToRaw(dst map[K]V) {
	maps.Copy(dst, *m)
}

func (m *GenMap[K, V]) CopyFrom(src GenMap[K, V]) {
	maps.Copy(*m, src)
}

func (m *GenMap[K, V]) CopyFromRaw(src map[K]V) {
	maps.Copy(*m, src)
}
